...
export function ProfileSection() {
  ...
  const lines: ReactNode[] = useMemo(
    () => [
      <>I'm <em className="font-serif not-italic">Kai</em> (Nguyen Le Truong Thien).</>,
      <>AI Engineer and <em className="font-serif not-italic">Full-Stack</em> Developer.</>,
      <>I work in <em className="font-serif not-italic">intelligent</em> systems and</>,
      <>web development, based in</>,
      <>Ho Chi Minh City, Vietnam,</>,
      <>with <em className="font-serif not-italic">AI</em> and data at the core.</>,
      <>I pursue creating experiences that</>,
      <>ignite <em className="font-serif not-italic">impact</em> and emotion,</>,
      <>like a <em className="font-serif not-italic">blue flame</em> in the heart.</>,
    ],
    []
  )
            ...
            {lines.map((node, i) => {
              ...
              return (
                <div key={i} className="relative overflow-hidden">
                  ...
                  <span className="block text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-light text-gray-900">{node}</span>
                  {/* White reveal overlay - clips from right (100%) to left (0%) */}
                  <motion.span
                    className="absolute inset-0 block text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-light text-white"
                    style={{
