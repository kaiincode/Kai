...
export function ServiceSection() {
  ...
  return (
    <section ref={sectionRef} className="relative">
      ...
      {/* Header + Intro - Reverted to original layout with faster reveal */}
      <div className="relative z-10 pt-24" style={{ paddingLeft: '10%', paddingRight: '10%' }}>
        ...
        {/* Intro text with faster horizontal reveal animation */}
        <div className="max-w-5xl mb-12">
          <div className="space-y-4">
            {introLines.map((node, i) => {
              // Faster reveal - shorter scroll ranges
              const lineStart = 0.05 + i * 0.04
              const lineEnd = lineStart + 0.08
              const lineReveal = useTransform(scrollYProgress, [lineStart, lineEnd], [0, 1])
              
              return (
                <div key={i} className="relative overflow-hidden">
                  {/* Base dark gray */}
                  <span className="block text-2xl sm:text-3xl lg:text-4xl xl:text-7xl leading-tight font-light text-gray-900">{node}</span>
                  {/* White reveal overlay */}
                  <motion.span
                    className="absolute inset-0 block text-2xl sm:text-3xl lg:text-4xl xl:text-7xl leading-tight font-light text-white"
                    style={{
        ...
        <div className="relative z-10 px-4 sm:px-8 lg:px-12 lg:ml-auto max-w-full lg:max-w-2xl">
          <div className="min-h-screen flex items-center text-white">
            <div>
              ...
              <p className="text-base sm:text-lg leading-relaxed font-light text-white/90 ml-4">I design agentic RAG systems with multi-agent orchestration, vector search (FAISS), and self-refinement loops. Specializing in citation-aware LLM fine-tuning and secure NLP pipelines for production-ready AI assistants.</p>
            </div>
          </div>
          <div className="min-h-screen flex items-center text-white">
            <div>
              <p className="text-xs tracking-[0.3em] font-light mb-6 text-white/60 -ml-4">02</p>
              <h3 className="text-3xl sm:text-4xl lg:text-5xl font-light mb-6 tracking-tight ml-4">Full-Stack Development</h3>
              <p className="text-base sm:text-lg leading-relaxed font-light text-white/90 ml-4">I engineer scalable e-commerce and analytics platforms using Next.js and Supabase. Expertise in implementing RBAC, real-time database models (PostgreSQL/Neo4j), and high-performance frontend architectures for complex management systems.</p>
            </div>
          </div>
          <div className="min-h-screen flex items-center text-white">
            <div>
